...
/**
 * Write the account. NOT reachable from the web — the only callers are setup()
 * below, which gates it behind ADMIN_SETUP_TOKEN, and tools/set-admin-password.js,
 * which is gated by having a shell on the server in the first place.
 *
 * Returns {ok} or {error}.
 */
function createAccount({ user, password, force }) {
  if (isConfigured() && !force) return { error: 'Admin account already exists.' };
  ...
  sessions.clear();
  return { ok: true, replaced: !!previous };
}

/**
 * Take the account straight from the environment, so there is no setup screen and no
 * setup token — you land on a normal login.
 *
 * ADMIN_USER + ADMIN_PASSWORD are authoritative: change them and restart, and the
 * password changes with it. That also means the account survives the host wiping the
 * home directory, which used to send you back to the setup screen.
 *
 * Runs once at boot. Returns a short status for the startup log; never logs or returns
 * the password itself.
 */
function bootstrapFromEnv() {
  const user = String(process.env.ADMIN_USER || '').trim();
  const password = String(process.env.ADMIN_PASSWORD || '');
  if (!user && !password) return { applied: false, reason: 'not-set' };
  if (!user || !password) {
    return { applied: false, reason: 'ADMIN_USER and ADMIN_PASSWORD must both be set' };
  }
  if (password.length < MIN_PASSWORD) {
    return {
      applied: false,
      reason: `ADMIN_PASSWORD is shorter than ${MIN_PASSWORD} characters — ignored`,
    };
  }

  // Already exactly this account? Then leave the stored record alone, so a restart
  // does not needlessly invalidate whoever is signed in.
  const current = readCreds();
  if (current && current.user === user
      && timingSafeEqual(hashPassword(password, current.salt), current.hash)) {
    return { applied: false, reason: 'already matches', configured: true };
  }

  const result = createAccount({ user, password, force: true });
  if (result.error) return { applied: false, reason: result.error };
  return { applied: true, replaced: !!current, user, configured: true };
}

/** One-time account creation from the web setup screen. Returns {ok} or {error}. */
  ...
  if (!setupTokenConfigured()) {
    return {
      error: 'Setup is disabled: ADMIN_SETUP_TOKEN is not set on the server ' +
        ...
        'variables and restart the app, then reload this page. If you have a shell ' +
        'on the server you can skip this screen entirely: run ' +
        '`node tools/set-admin-password.js` there instead.',
    };
  }
...
module.exports = {
  ...
  isConfigured, setupTokenConfigured, setup, createAccount, bootstrapFromEnv, login, logout,
