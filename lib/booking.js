...
/* 'enquiry': the site just collects the job and Roy calls back — no calendar, no held
 * slot. 'booking': the full calendar/slot engine, as it always worked. This is meant
 * to flip back and forth as Roy's workload changes, so both paths stay live code. */
const MODES = ['enquiry', 'booking'];
...
const DEFAULTS = {
  enabled: true,
  /* Default 'enquiry' on purpose: a live settings.json saved before this switch
   * existed has no `mode` key at all, and it must come back as 'enquiry' too — see
   * normalizeSettings(), which falls back to DEFAULTS.mode whenever the value is
   * missing or not one of MODES. */
  mode: 'enquiry',
  blocks: [
  ...
  // Separate wording for enquiry mode, so switching modes never shows the booking
  // copy ("Pick a time that suits you…") when there is no time to pick.
  enquiryTitle: 'Send an enquiry',
  enquiryBlurb: "Tell me about the job and I'll get back to you — usually the same day.",
  /* The wording of the confirmation email a customer gets once Roy approves their
   * request. Only the words live here — the branding around them is in
   * lib/mail-templates.js. {{token}} placeholders are listed there in TOKEN_HELP. */
  confirmEmail: {
    enabled: true,
    subject: "You're booked in for {{date}} — Wilkin Plumbing",
    intro: 'Hi {{first_name}},\n\n'
      + "That's confirmed — I'll see you {{when}}. Everything you sent me is below; "
      + 'give it a quick look and tell me if anything is wrong.',
    closing: 'If you need to move it, or something changes with the job, just reply to '
      + 'this email or give me a ring and we\'ll sort it out.\n\nThanks,\nRoy',
  },
};
...
function normalizeSettings(raw) {
  ...
  // Anything other than the two known values — missing (an old settings.json),
  // mistyped, tampered — resolves to the safe default, 'enquiry'.
  s.mode = MODES.includes(s.mode) ? s.mode : DEFAULTS.mode;
  ...
  s.enquiryTitle = String(s.enquiryTitle || DEFAULTS.enquiryTitle).trim().slice(0, 80)
    || DEFAULTS.enquiryTitle;
  s.enquiryBlurb = String(s.enquiryBlurb || DEFAULTS.enquiryBlurb).trim().slice(0, 400)
    || DEFAULTS.enquiryBlurb;

  /* Confirmation-email wording. Falling back to the default on a blank field matters:
   * an empty subject line would otherwise ship a real email with no subject. */
  const ce = (s.confirmEmail && typeof s.confirmEmail === 'object') ? s.confirmEmail : {};
  s.confirmEmail = {
    enabled: ce.enabled === undefined ? DEFAULTS.confirmEmail.enabled : !!ce.enabled,
    subject: cleanMultiline(ce.subject, 160).replace(/\n+/g, ' ').trim()
      || DEFAULTS.confirmEmail.subject,
    intro: cleanMultiline(ce.intro, 1200).trim() || DEFAULTS.confirmEmail.intro,
    closing: cleanMultiline(ce.closing, 1200).trim() || DEFAULTS.confirmEmail.closing,
  };
  return s;
}
...
/** writeSettings without the write — what the admin's email preview renders against, so
 *  Roy can see an unsaved draft exactly as it would send. */
function previewSettings(patch) {
  return normalizeSettings({ ...readSettings(), ...(patch || {}) });
}
 ...
 * "not open".
 *
 * In enquiry mode there is no calendar to show — `days` comes back empty and the
 * title/blurb are the enquiry wording, not the booking wording. `enabled` still means
 * what it always did (the on/off switch), independent of which mode is selected.
 */
function publicAvailability() {
  const settings = readSettings();

  if (settings.mode === 'enquiry') {
    return {
      enabled: settings.enabled,
      mode: settings.mode,
      title: settings.enquiryTitle,
      blurb: settings.enquiryBlurb,
...
/** @param {{requireSlot?: boolean}} [opts] — pass {requireSlot:false} for an enquiry,
 *  where there is no date/block to validate. Everything else is checked the same way
 *  whichever mode this came in through: address, name, phone and job stay required. */
function validate(input, opts) {
  const requireSlot = !(opts && opts.requireSlot === false);
  const rec = {
  ...
  if (requireSlot && !isIsoDate(rec.date)) return { error: 'Please choose a date.' };
  return { rec };
}
 ...
 *
 * The enquiry/booking switch only changes what the *public site* asks for: when Roy
 * adds a job himself (source 'admin') he always books a real date, because he has
 * already agreed the visit on the phone — whatever the site is currently set to.
 */
function createRequest(input, source = 'site') {
  ...
  const enquiry = settings.mode === 'enquiry' && source === 'site';

  const checked = validate(input || {}, { requireSlot: !enquiry });
  ...
  const record = {
    ...
    updatedAt: now,
    status: 'pending',
    // An enquiry never holds a slot, whatever the caller labelled it coming in — mark
    // it plainly so the admin can always tell a dateless enquiry from a real booking.
    source: enquiry ? 'enquiry' : source,
    date: enquiry ? null : rec.date,
    block: enquiry ? null : rec.block,
    name: rec.name,
    phone: rec.phone,
    email: rec.email,
    address: rec.address,
...
function updateRequest(id, patch) {
  ...
  // previousStatus lets the caller spot the pending → confirmed edge and email the
  // customer once, rather than every time Roy saves a note on an already-confirmed job.
  return { record: next, previousStatus: before.status, settings };
}
...
/** Newest first, with resolved jobs pushed behind the live ones. Within a status,
 *  dateless records (enquiries) have nothing to schedule by, so they sort ahead of
 *  any dated job, newest first — the dated ones keep their existing date/block order. */
function listRequests() {
  ...
  return readRequests().slice().sort((a, b) => {
    ...
    const aDated = isIsoDate(a.date);
    const bDated = isIsoDate(b.date);
    if (aDated !== bDated) return aDated ? 1 : -1;
    if (!aDated) return String(b.createdAt).localeCompare(String(a.createdAt));
...
module.exports = {
  DIR, SETTINGS_FILE, REQUESTS_FILE, DEFAULTS, WEEKDAYS, STATUSES, MODES, TZ,
  todayIso, addDays, weekdayOf, isIsoDate, prettyDate,
  readSettings, writeSettings, previewSettings, readRequests, writeRequests,
