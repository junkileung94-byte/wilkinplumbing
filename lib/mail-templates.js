...
function tokensFor(record, settings) {
  ...
  const slot = block
    ...
    : String(record.block || '');
  // Only ever called for a record that actually has an appointment (confirmation(),
  // and the sample preview) — a dateless enquiry never reaches this. Guarded anyway,
  // rather than handing a null through to booking.prettyDate() and risking it printing
  // "Invalid Date" or the literal string "null" into a customer-facing email.
  const day = record.date ? booking.prettyDate(record.date) : '';
  return {
    name: record.name || '',
