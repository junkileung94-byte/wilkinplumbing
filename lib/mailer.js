 ...
 * and nothing else. No attachments, no HTML, no queue.
 *
...
function buildMessage({ from, to, subject, text, replyTo }) {
  const body = Buffer.from(String(text == null ? '' : text).replace(/\r?\n/g, '\r\n'), 'utf8')
...
async function send({ to, subject, text, replyTo }) {
  ...
  try {
    ...
    const message = buildMessage({ from: c.from, to: rcpt, subject, text, replyTo });
